...
const initializeWasm = (env) => {
    ...
    const rawUuid = (env.UUID || defaultUuid).trim();
...
const establishTcpConnection = async (parsedRequest, request) => {
    ...
    if (clean.length < 6) {
        ...
        const urlBytes = textEncoder.encode(clean), writeLen = Math.min(urlBytes.length, 4096);
        wasmMem.set(urlBytes.subarray(0, writeLen), urlPtr);
        parseUrlWasm(writeLen);
